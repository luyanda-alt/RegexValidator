...
public class RegularExpression 
{
	...
	// Generates the NFA using the regular expression
	public static NFA generateNFA(String regular) 
	{
		// Generates the regular expression with the concatenation
		regular = AddConcat (regular);
		
		// The only symbols of the alphabet
		input.add('a');
		input.add('b');
	...
	// Perform the desired operation based on the top of stackNfa
	private static void doOperation () 
	{
		if (RegularExpression.stateID > 0) 
		{
			...
			{
				case ('|'):
					union(); //Call the correct method
					break;
	
				case ('.'):
					concatenation(); //Call the correct method
					break;
	
				case ('*'):
					star(); //Call the correct method
					break;
	...
	private static void concatenation() 
	{
		...
		// Place transition from the end of nfa 1 to the start of nfa 2
	...
	// Creates the union of sub NFA 1 with sub NFA 2
	private static void union() 
	{
		// Load two NFAs in stack into variables
		NFA nfa2 = stackNfa.pop();
		NFA nfa1 = stackNfa.pop();
	...
	// add "." when there is concatenation between two symbols.
	private static String AddConcat(String regular) 
	{
		...
		for (int i = 0; i < regular.length() - 1; i++) 
		{
			...
			else if ( isInputCharacter(regular.charAt(i)) && regular.charAt(i+1) == 'e') 
			{
				newRegular += regular.charAt(i) + ".";
				
			} 
			else if ( regular.charAt(i) == 'e' && isInputCharacter(regular.charAt(i+1)) ) 
			{
				newRegular += regular.charAt(i) + ".";
	...
	private static void removeEpsilonTransition() 
	{
		...
		while (!stack.isEmpty()) 
		{
			...
			{
				// Check if p is in the set otherwise add it to the stack
				if (!set2.contains(p)) 
				{
					set2.add(p);
					stack.push(p);
	...
	// Move states based on the input symbol
